"""
Service Level Specific Extras Testing (Async Version)
...
Usage:
    ...
    python service_level_analyzer.py --carrier usps --concurrency 10
"""
...
# =============================================================================
# Detailed Service Level Tester (Async)
# =============================================================================

class ServiceLevelExtrasAnalyzer:
    """
    Analyzes extras support at the service level granularity (async version).
    ...
    async def _get_baseline_rate(
        self,
        client: ShippoClient,
        carrier_account_id: str,
        service_level: str
    ) -> Optional[dict]:
        """Get baseline rate for a service level (with caching)"""
        cache_key = f"{carrier_account_id}:{service_level}"
    ...
    async def _test_extra_at_service_level(
        self,
        client: ShippoClient,
    ...
    async def _analyze_service_level_batch(
        self,
        client: ShippoClient,
        carrier: str,
        carrier_account_id: str,
        ...
        baseline_rate: dict
    ) -> tuple[ServiceLevelMatrix, list[ServiceLevelTestResult]]:
        """Analyze all extras for a single service level in parallel"""

        # Create tasks for all extras
        tasks = [
            self._test_extra_at_service_level(
                client=client,
                extra_def=extra_def,
                carrier=carrier,
                service_level=service_level,
        ...
        for i, result in enumerate(results):
            extra_def = extras_to_test[i]

            if isinstance(result, Exception):
                error_result = ServiceLevelTestResult(
                    carrier=carrier,
                    service_level=service_level,
                    service_category=category,
                    extra_name=extra_def.name,
        ...
        return matrix, processed_results

    async def analyze_carrier_service_levels(
        self,
        carrier: str,
        carrier_account_id: str,
        ...
        async with ShippoClient(self.api_key, self.concurrency) as client:
            for sl in service_levels:
                ...
                # Get baseline
                baseline = await self._get_baseline_rate(client, carrier_account_id, sl_token)

                if baseline is None:
                    print("    Baseline failed - skipping service level")
                    continue

                # Run all extras for this service level in parallel
                matrix, results = await self._analyze_service_level_batch(
                    client=client,
                    carrier=carrier,
                    carrier_account_id=carrier_account_id,
                ...
                # Print summary
                print(f"    ✓{len(matrix.fully_supported)} $±{len(matrix.rate_impacting)} "
                      f"✗{len(matrix.rejected)} ○{len(matrix.ignored)} ⚠{len(matrix.errors)}")
...
async def async_main(args):
    ...
    # Analyze
    analyzer = ServiceLevelExtrasAnalyzer(api_key, concurrency=args.concurrency)
    all_matrices = []
    ...
    for carrier, info in carriers.items():
        ...
        matrices = await analyzer.analyze_carrier_service_levels(
            carrier=carrier,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Analyze service level specific extras (async)")
